...
#   HMreset_win .text
#
# Mucked with For gdbTk by Jim Ingham - 4/8/98
#
...
# This is the hand cursor that is assumed when the pointer enters a link:

set HMhandCursor hand2
...
set HMtag_map(hmstart) {
	family times   weight normal   style roman   size 14
...
proc HMinit_win {win} {
    ...
    # hypertext links
	$win tag configure link -borderwidth 0 -foreground blue	-underline 1
...
proc HMset_state {win args} {
    ...
    if {![llength $args] % 2} { return 0}
		...
		incr bad [catch {set var($HMparam_map($name)) $val}]
	}
  return [expr {$bad == 0}]
}
...
proc HMrender {win tag not param text} {
    ...
    if {$var(S_stop)} { puts "Got stop"; return }
...
# JCI - I took out all the HMevents bindings, and replaced them with
# HMlink_???_callback functions. 
# which looks nicer IMHO.
...
proc HMlink_setup {win href} {
    ...
    $win tag bind  L:$href <Enter> [list HMlink_enter_callback $win $href]
    $win tag bind  L:$href <Leave> [list HMlink_leave_callback $win $href]

 
}

# Generic link-enter callback
# users are supposed to override this to provide their own
# behavior when the cursor enters and leaves links
# Added by JCI

if {[info command HMlink_enter_callback] == ""} {
    proc HMlink_enter_callback {win href} {
	global HMoldCursor HMhandCursor
	
	set HMoldCursor [$win cget -cursor]
	$win configure -cursor $HMhandCursor
	
    }
}

if {[info command HMlink_leave_callback] == ""} {
    proc HMlink_leave_callback {win href} {
	global HMoldCursor
	
	$win configure -cursor $HMoldCursor
...
if {[info command HMlink_callback] == ""} {
    proc HMlink_callback {win href} {
...
proc HMcurrent_tags {win} {
	...
	HMcreate_font $font $family $size $weight $style $var(S_adjust_size)
    }
	HMset_font $win $font
...
proc HMset_font {win font} {
	catch {$win tag configure $font -font $font} msg
}

# Create a Tk8.0 named font
proc HMcreate_font {font family size weight style {adjust_size 0}} {
    catch {incr size $adjust_size}
    if {$size == ""} {
	set size 0
    }
    font create $font -family $family -size $size -weight $weight -slant $style
}

# This clears all the font objects that the html_library has allocated.
proc HMclear_fonts {} {
    foreach font [font names] {
	if {[string match HM* $font]} {
	    font delete $font
	}
    }
}

# This resets the base font types (fixed and proportional) to the 
# attributes from the two font objects passed in.

proc HMreset_fonts {fixed prop} {
    global HMtag_map

    HMclear_fonts
    
# First fo the fixed font
    set family [font configure $fixed -family]

    HMkeyl_set HMtag_map(code) family $family
    HMkeyl_set HMtag_map(kbd)  family $family
    HMkeyl_set HMtag_map(pre)  family $family
    HMkeyl_set HMtag_map(samp) family $family
    HMkeyl_set HMtag_map(tt)   family $family

# Now the proportional font...

    set family [font configure $prop -family]
    set size [font configure $prop -size]

    HMkeyl_set HMtag_map(hmstart) family $family
    HMkeyl_set HMtag_map(hmstart) size $size
    HMkeyl_set HMtag_map(h5)      size [incr size 2]    
    HMkeyl_set HMtag_map(h4)      size [incr size 2]    
    HMkeyl_set HMtag_map(h3)      size [incr size 2]    
    HMkeyl_set HMtag_map(h2)      size [incr size 2]    
    HMkeyl_set HMtag_map(h1)      size [incr size 2]    

}

# This is a cheesy replacement for the set function in TclX's
# keyed list facility.
    
proc HMkeyl_set {listPtr inKey inVal} {
    upvar $listPtr list

    set indx 1
    foreach {key val} $list {
	if {[string compare $key $inKey] == 0} {
	    set list [lreplace $list $indx $indx $inVal]
	    return
	}
	incr indx 2
    }
    
    lappend list $inKey $inVal
}
...
proc HMparse_html {html {cmd HMtest_parse} {start hmstart}} {
	...
	set sub "\}\n$cmd {\\2} {\\1} {\\3} \{"
        set tags_found [regsub -all $exp $html $sub html]
        HMrender_start $cmd $tags_found
	eval "$cmd {$start} {} {} \{ $html \}"
	eval "$cmd {$start} / {} {}"
        HMRender_end $cmd
}

if {[info commands HMrender_start] == ""} {
    proc HMrender_start {cmd tags} {
    }
}

if {[info commands HMrender_end] == ""} {
...
# Make sure that itcl_mkindex does not pick this proc up...
# Don't remove this ::, as it fools itcl_mkindex into not
# indexing this.  Otherwise if gdbtk_library is ahead of
# tk_library on the auto_path, this will cause an infinite
# recursion.  
