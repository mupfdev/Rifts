...
itcl_class ActionDlg {
  ...
  constructor {config} {
    ...
    lappend Variables "All Arguments" "All Locals" "All Registers"
  ...
  method sort {list} {
    ...
    set locs 0
    set regs 0
    set i [lsearch $list "All Arguments"]
    if {$i != -1} {
      set args 1
      set list [lreplace $list $i $i]
    }

    set i [lsearch $list "All Locals"]
    if {$i != -1} {
      set locs 1
      set list [lreplace $list $i $i]
    }

    set i [lsearch $list "All Registers"]
    if {$i != -1} {
      set regs 1
      set list [lreplace $list $i $i]
    }
    ...
    set argmuments [lsort $arguments]
    ...
    set list [concat $arguments $locals $globals $registers]
