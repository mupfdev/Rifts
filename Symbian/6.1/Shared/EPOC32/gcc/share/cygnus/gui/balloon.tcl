...
itcl_class Balloon {
  ...
  constructor {top} {
    ...
    @@toplevel $this -class $class -borderwidth 1 -background black
    ...
    label $this.label -background $bg -foreground $fg -font global/status \
    ...
    bind $this.label <Destroy> [list $this delete]
  }
  ...
  method showballoon {W tag} {
    ...
    if {[info exists left] && $help != ""} then {
      $this.label configure -text $help
      set lw [winfo reqwidth $this.label]
	...
	set left [expr {$sw - 2 * $bw - $lw}]
      ...
      set lh [winfo reqheight $this.label]
      if {$ypos + $lh >= [winfo screenheight $this]} then {
	set ypos [expr {$alt_ypos - $lh}]
      }

      wm positionfrom $this user
      wm geometry $this +${left}+${ypos}
      update
      wm deiconify $this
      raise $this
...
proc BALLOON_command_show {window {tag {}}} {
  set b [BALLOON_find_balloon $window]
  $b showballoon $window $tag
}
