...
itcl_class Sendpr {
  ...
  constructor {{text ""}} {
    ...
    # style guidelines.
    @@toplevel $this -class $class -borderwidth 7 -relief flat
    ...
    Labelledframe $this.cframe -text [gettext "Classification"]
    set parent [$this.cframe get_frame]
    ...
    Labelledframe $this.synopsis -text [gettext "Synopsis"]
    set parent [$this.synopsis get_frame]
    ...
    # just confusing.
    @@set SENDPR_state($this,repeat) [_make_text $this.desc \
					[gettext "Description"]]

    # Some buttons.
    frame $this.buttons -borderwidth 0 -relief flat
    button $this.buttons.send -text [gettext "Send"] \
      -command [list $this _send]
    button $this.buttons.cancel -text [gettext "Cancel"] \
      -command [list destroy $this]
    button $this.buttons.help -text [gettext "Help"] -state disabled
    standard_button_box $this.buttons

    # FIXME: we'd really like to have sashes between the text widgets.
    # iwidgets or tix will provide that for us.
    grid $this.cframe -sticky ew -padx 4 -pady 4
    grid $this.synopsis -sticky ew -padx 4 -pady 4
    grid $this.desc -sticky news -padx 4 -pady 4
    grid $this.buttons -sticky ew -padx 4

    grid rowconfigure $this 0 -weight 0
    grid rowconfigure $this 1 -weight 0
    grid rowconfigure $this 2 -weight 1
    grid rowconfigure $this 3 -weight 1
    grid columnconfigure $this 0 -weight 1

    bind $this.buttons <Destroy> [list $this delete]
