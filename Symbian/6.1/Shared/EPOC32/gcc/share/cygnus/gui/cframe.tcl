...
itcl_class Checkframe {
  ...
  public help {} {
    if {[winfo exists $this.check]} then {
      balloon register $this.check $help
    }
  }
  ...
  constructor {config} {
    ...
    checkbutton $this.check -text $text -variable $variable -padx 2 \
  ...
  method _set_visibility {{child {}}} {
	...
	set avoid($child) {}
      ...
      foreach child [winfo children $this.iframe.frame] {
  ...
  method _set_option {option value {set_vis 1}} {
    if {[winfo exists $this.check]} then {
      $this.check configure $option $value
      if {$set_vis} then {
	_set_visibility
      }
    }
