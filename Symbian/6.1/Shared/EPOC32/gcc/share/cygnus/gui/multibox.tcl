...
itcl_class Multibox {
  ...
  constructor {config} {
    ...
    scrollbar $this.vs -orient vertical
    bind $this.vs <Destroy> [list $this delete]
  ...
  method add {args} {
    ...
    listbox $this.box$num -exportselection 0 -height $height \
      -selectmode $selectmode -width $opts(-width)
    if {$num == 0} then {
      $this.box$num configure -yscrollcommand [list $this.vs set]
      $this.vs configure -command [list $this yview]
    }
    label $this.label$num -text $opts(-title) -anchor w

    # No more class bindings.
    set tag_list [bindtags $this.box$num]
    set index [lsearch -exact $tag_list Listbox]
    bindtags $this.box$num [lreplace $tag_list $index $index Multibox]

    grid $this.label$num -row 0 -column $num -sticky new
    grid $this.box$num -row 1 -column $num -sticky news
    if {$opts(-fix)} then {
      grid columnconfigure $this $num -weight 0 \
	-minsize [winfo reqwidth $this.box$num]
    } else {
      grid columnconfigure $this $num -weight 1
    }

    lappend _listboxen $this.box$num

    # Move the scrollbar over.
    incr num
    grid $this.vs -row 1 -column $num -sticky nsw
    grid columnconfigure $this $num -weight 0
  }
