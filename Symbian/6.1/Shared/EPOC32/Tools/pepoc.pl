...
sub launchPepoc
{
	...
	. "release\\wins\\" . $type . "\\" . "pepoc.exe";
    -e $emu ||
	die "ERROR: File \"$emu\" not found.\n\n" .
	    "The EPOCROOT environment variable does not identify\n" .
	    "a valid pepoc installation on this drive.\n" . 
		"EPOCROOT must be an absolute path to an existing\n" .
		    "directory - it should have no drive qualifier and\n" .
			"must end with a backslash.\n";
			
    # Keep hold of console until emulator is closed
    if (system($emu))
	{
	die "Failed to execute pepoc \"$emu\": $!";
	}
}

sub printHelp
{
    print "Pepoc Launcher\n";
    print "Syntax :       pepoc [-rel -help]\n";
    print "(no options)   Launch active debug pepoc\n";
    print "-rel           Launch active release pepoc\n";
