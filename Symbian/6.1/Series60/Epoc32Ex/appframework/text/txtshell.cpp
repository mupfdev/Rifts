...
void CExampleShellContainer::NotifyGraphicExampleFinished()
	{
	_LIT(KStatus,"example finished");
	NotifyStatus(KStatus);
	}
...
void CExampleShellAppUi::ConstructL()
	...
	iContainer->NotifyStatus(KStatus);
...
void CExampleShellAppUi::HandleCommandL(TInt aCommand)
	...
	switch (aCommand)
		{
	case EExampleShellSelectGlobal:
		iContainer->ResetExampleL(new (ELeave) CGlobalControl);
		return;
	case EExampleShellSelectRich:
		iContainer->ResetExampleL(new (ELeave) CRichControl);
		return;
	case EExampleShellSelectStyled:
		iContainer->ResetExampleL(new (ELeave) CStyleControl);
		return;
	case EExampleShellSelectView:
		iContainer->ResetExampleL(new (ELeave) CViewControl);
		return;
	case EEikCmdExit:
		Exit();
		return;
