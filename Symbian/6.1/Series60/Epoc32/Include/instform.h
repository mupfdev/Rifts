...
enum TInstFileTextOption
	...
	EInstFileTextOptionSkipOneIfNo,		// Yes/No - skip next file if user selects No
	EInstFileTextOptionAbortIfNo,		// Yes/No - abort whole install if user selects No
	EInstFileTextOptionExitIfNo			// Yes/No - uninstall if user selects No
	};

enum TInstFileRunOption
	{
	EInstFileRunOptionInstallOnly		= 0,	// Only run on installation
	EInstFileRunOptionRemoveOnly		= 1<<0,	// Only run on de-installation
	EInstFileRunOptionInstallAndRemove	= 1<<1,	// Run on install and de-install
	};
...
enum TVariableToken
	...
	// start for option1, option2,... variables
